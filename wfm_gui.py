...
class WFMLauncher:
    def __init__(self, root):
        self.root = root
        self.root.title("WFM Multi-Skill Launcher v3.1 (Smart Balance)")
        ...
        self.guarantee_min_days_var = tk.BooleanVar(value=True)
        self.overstaff_distribution_var = tk.StringVar(value="Smart Balance (Consigliato)")
        self.overstaff_distribution_options = [
            "Smart Balance (Consigliato)",
            "Distribuito Proporzionale",
            "Weekend Ammesso"
        ]
        self.weekday_overcap_max_pct = tk.IntVar(value=20)
    ...
    def _build_ui(self):
        ...
        # --- Politica Giorni Minimi ---
        mindays_frame = ttk.LabelFrame(config_frame, text="Politica Giorni Minimi", padding="8")
        mindays_frame.pack(fill=tk.X, pady=5)

        mindays_row1 = ttk.Frame(mindays_frame)
        mindays_row1.pack(fill=tk.X, pady=2)
        ttk.Checkbutton(
            mindays_row1,
            text="Garantisci giorni minimi contrattuali",
            variable=self.guarantee_min_days_var,
            command=self._toggle_mindays_options,
        ).pack(side=tk.LEFT)
        ttk.Label(mindays_row1, text="(se disattivo, comportamento 'Efficienza pura')", font=("Segoe UI", 8, "italic")).pack(side=tk.LEFT, padx=(10, 0))

        mindays_row2 = ttk.Frame(mindays_frame)
        mindays_row2.pack(fill=tk.X, pady=2)
        self.mindays_label1 = ttk.Label(mindays_row2, text="Distribuzione overstaff:")
        self.mindays_label1.pack(side=tk.LEFT)
        self.mindays_combo = ttk.Combobox(
            mindays_row2,
            textvariable=self.overstaff_distribution_var,
            values=self.overstaff_distribution_options,
            state="readonly",
            width=30
        )
        self.mindays_combo.pack(side=tk.LEFT, padx=5)
        self.mindays_label2 = ttk.Label(mindays_row2, text="Max overstaff Lun-Ven (%):")
        self.mindays_label2.pack(side=tk.LEFT, padx=(15, 5))
        self.mindays_scale = tk.Scale(
            mindays_row2,
            from_=0,
            to=50,
            orient=tk.HORIZONTAL,
            ...
            showvalue=True,
            length=120,
            variable=self.weekday_overcap_max_pct,
        )
        self.mindays_scale.pack(side=tk.LEFT)

        ttk.Label(
            mindays_frame,
            text="Smart Balance: overstaff prima su Lun-Ven, weekend solo se necessario. Distribuito: proporzionale alla domanda. Weekend Ammesso: comportamento classico.",
        ...
        quick_text = (
            "Suggerimenti rapidi:\n"
            "A) Proteggere weekend + giorni minimi: Garantisci giorni ON + Smart Balance + Weekend guard ON + Max 20-30%.\n"
            "B) Efficienza pura (meno overstaff): Garantisci giorni OFF, Weekend guard ON (0-10%), Overcap Normale/Rigida.\n"
            "C) Orari puliti: Rigida/Obbligata, ma rischio gap e meno copertura.\n"
            "D) Overstaff uniforme: Distribuito Proporzionale + attiva 'Overstaff uniforme'.\n"
    ...
    def _toggle_mindays_options(self):
        """Abilita/disabilita le opzioni di distribuzione overstaff in base al checkbox."""
        if self.guarantee_min_days_var.get():
            state = "normal"
            combo_state = "readonly"
        else:
            state = "disabled"
            combo_state = "disabled"
        self.mindays_combo.config(state=combo_state)
        self.mindays_scale.config(state=state)
    ...
    def _build_command(self, output_path, skill_name=None):
        ...
        # Politica Giorni Minimi
        if self.guarantee_min_days_var.get():
            args.append("--guarantee-min-days")
            dist = self.overstaff_distribution_var.get()
            if "Smart Balance" in dist:
                args.append("--smart-balance")
            elif "Distribuito" in dist:
                args.append("--overstaff-distributed")
            # "Weekend Ammesso" = nessun flag aggiuntivo (comportamento default)

            weekday_max = int(self.weekday_overcap_max_pct.get() or 0)
            if weekday_max > 0:
                args.extend(["--weekday-overcap-max", str(weekday_max)])
...
13. POLITICA GIORNI MINIMI (NUOVO)
   Gestisce il trade-off tra rispettare i giorni minimi contrattuali e limitare l'overstaff.

   - Checkbox "Garantisci giorni minimi contrattuali":
     Se attivo, il sistema cerca di assegnare a tutti il numero di giorni previsto dal contratto.
     Se disattivo, comportamento "Efficienza pura" (alcuni potrebbero lavorare meno giorni).

   - Distribuzione overstaff:
     * Smart Balance (Consigliato): L'overstaff va PRIMA sui giorni Lun-Ven. Il weekend viene
       toccato solo se Lun-Ven e pieno. Ideale per proteggere il weekend.
     * Distribuito Proporzionale: L'overstaff viene distribuito su tutti i giorni in modo
       proporzionale alla domanda (stesso rapporto copertura/domanda su ogni giorno).
     * Weekend Ammesso: Comportamento classico, l'overstaff puo andare ovunque (anche weekend).

   - Max overstaff Lun-Ven (%): Limita quanto overstaff puo andare sui giorni infrasettimanali.
     Es: 20% significa che Lun-Ven possono avere al massimo 20% di personale in piu rispetto alla domanda.

   Scenari consigliati:
   - Proteggere weekend + rispettare giorni minimi: Smart Balance + Weekend guard ON + Max 20-30%
   - Overstaff uniforme su tutti i giorni: Distribuito Proporzionale + Overstaff uniforme ON
   - Massima flessibilita: Weekend Ammesso (come il vecchio Bilanciamento)
