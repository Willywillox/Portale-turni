...
def assegnazione_tight_capacity(
    ...
    guarantee_min_days: bool = False,
    smart_balance: bool = False,
    overstaff_distributed: bool = False,
    weekday_overcap_max: Optional[float] = None,
):
    ...
    # Politica Giorni Minimi
    guarantee_min_days = bool(guarantee_min_days)
    smart_balance = bool(smart_balance)
    overstaff_distributed = bool(overstaff_distributed)
    if weekday_overcap_max is not None:
        try:
            weekday_overcap_max = float(weekday_overcap_max)
        except (TypeError, ValueError):
            weekday_overcap_max = None
    if weekday_overcap_max is not None and weekday_overcap_max <= 0:
        weekday_overcap_max = None
    # Default weekday_overcap_max se smart_balance attivo
    if smart_balance and weekday_overcap_max is None:
        weekday_overcap_max = 0.20  # 20% default
    ...
    def apply_assignment(emp: str, day: str, sid: str, forced: bool = False, force_any_duration: bool = False) -> None:
        # FIX: Controlla se il dipendente è già assegnato a questo giorno
        if day in assignments_by_emp.get(emp, {}):
            # Già assegnato a questo giorno - non fare nulla (evita double-count)
            return
        ...
        if demand_overlap_guard and demand_by_day.get(day, 0.0) > 0 and not force_any_duration:
        ...
        if remaining_duration_counts.get(emp, Counter()).get(duration_needed, 0) <= 0:
            # FIX: se force_any_duration (fallback finale per garantire giorni minimi), permetti comunque
            if not force_any_duration:
        ...
        if out_of_range_flag and out_of_range_used[emp] >= out_of_range_allowance.get(emp, 0):
            # FIX: se force_any_duration (fallback finale), permetti comunque per garantire giorni minimi
            if not force_any_duration:
    ...
    def weekday_overcovered(day: str) -> bool:
        """Controlla se un giorno infrasettimanale ha superato il limite di overstaff."""
        if day in weekend_days:
            return False  # Usa weekend_overcovered per weekend
        if not smart_balance or weekday_overcap_max is None:
            return False
        day_demand = demand_by_day.get(day, 0.0)
        if day_demand <= 0:
            return False
        total_coverage = sum(current_coverage[day].values())
        allowed = day_demand * (1.0 + weekday_overcap_max)
        return total_coverage >= allowed

    def pick_force_assignment(emp: str):
        """Restituisce (day, sid, used_final_fallback) - used_final_fallback indica se è stato usato il fallback estremo."""
        if not force_balance and not guarantee_min_days:
        ...
        # Smart Balance: prima SOLO infrasettimanali, poi weekend come fallback
        if smart_balance:
            # Prima passata: solo infrasettimanali con domanda
            ordered_days_pass1 = weekdays
            # Seconda passata (fallback): weekend
            ordered_days_pass2 = weekends + [g for g in giorni if g in zero_demand_days]
        else:
            # Comportamento originale: tutti insieme
            ordered_days_pass1 = weekdays + weekends + [g for g in giorni if g in zero_demand_days]
            ordered_days_pass2 = []

        force_on_days = forced_on.get(emp, set())

        def try_days(day_list, allow_weekday_overcap=False, ignore_demand_guard=False, force_any_shift=False):
            nonlocal best, best_day, best_sid
            for day in day_list:
                if day in forced_off.get(emp, set()):
                    continue
                if (emp, day) in assigned_once:
                    continue
                # In fallback finale (ignore_demand_guard), permetti anche weekend overcovered
                if day in weekend_days and weekend_overcovered(day):
                    if not ignore_demand_guard:
                        continue
                # Smart Balance: controlla limite infrasettimanale
                if smart_balance and day not in weekend_days:
                    if not allow_weekday_overcap and weekday_overcovered(day):
                        continue
                for sid in get_shifts(emp, day):
                    # FIX: nel fallback finale (force_any_shift), bypassa can_use_shift per garantire giorni minimi
                    if not force_any_shift and not can_use_shift(emp, sid, day):
                        continue
                    ratio, with_demand, _ = shift_overlap_info(day, sid)
                    # FIX: se ignore_demand_guard, permetti turni senza domanda
                    if demand_overlap_guard and with_demand == 0 and not ignore_demand_guard:
                        continue
                    val = shift_value(emp, day, sid, allow_overcapacity=True, force_any=force_balance or guarantee_min_days)
                    if demand_overlap_guard and not ignore_demand_guard:
                        val -= (1.0 - ratio) * 120.0
                    if val <= -1e4 and not ignore_demand_guard:
                        continue
                    if day in force_on_days:
                        val += 220.0
                    # Smart Balance: bonus per infrasettimanali
                    if smart_balance and day not in weekend_days:
                        val += 100.0
        ...
        # Prima passata: cerca nei giorni primari
        try_days(ordered_days_pass1, allow_weekday_overcap=False)

        # Se Smart Balance e non trovato, prova infrasettimanali con overcap
        if smart_balance and best is None:
            try_days(ordered_days_pass1, allow_weekday_overcap=True)

        # Seconda passata (fallback): weekend/altri
        if best is None and ordered_days_pass2:
            try_days(ordered_days_pass2, allow_weekday_overcap=True)

        # TERZA PASSATA (ultima risorsa): ignora demand_overlap_guard E can_use_shift per rispettare giorni minimi
        if guarantee_min_days and best is None:
            all_days = weekdays + weekends + [g for g in giorni if g in zero_demand_days]
            try_days(all_days, allow_weekday_overcap=True, ignore_demand_guard=True, force_any_shift=True)
            if best is not None:
                used_final_fallback = True

        return best_day, best_sid, used_final_fallback
    ...
    while remaining_people() and safety < 12000:
        ...
        for emp in remaining_people():
            ...
            if best_sid is not None:
                ...
                if force_balance or guarantee_min_days:
                    fallback_day, fallback_sid, used_final_fallback = pick_force_assignment(emp)
                    if fallback_sid is not None:
                        apply_assignment(emp, fallback_day, fallback_sid, forced=True, force_any_duration=used_final_fallback)
                        assigned_flag = True
                    else:
    ...
    def ensure_required_days():
        ...
        while True:
            ...
            progress = False
            for emp in missing:
                # Usa pick_force_assignment se force_balance O guarantee_min_days
                used_final_fallback = False
                if force_balance or guarantee_min_days:
                    day_sel, sid_sel, used_final_fallback = pick_force_assignment(emp)
                else:
                    best = None
                    day_sel = None
                    sid_sel = None
                    # Smart Balance: ordina giorni con infrasettimanali prima
                    if smart_balance:
                        weekdays_valid = [g for g in giorni_validi if g not in weekend_days]
                        weekends_valid = [g for g in giorni_validi if g in weekend_days]
                        days_to_try = weekdays_valid + weekends_valid
                    else:
                        days_to_try = giorni_validi
                    for day in days_to_try:
                        ...
                        if weekend_overcovered(day):
                            continue
                        # Smart Balance: controlla limite infrasettimanale
                        if smart_balance and day not in weekend_days and weekday_overcovered(day):
                            continue
                        for sid in get_shifts(emp, day):
                            if not can_use_shift(emp, sid, day):
                                continue
                ...
                if sid_sel is None:
                    continue
                apply_assignment(emp, day_sel, sid_sel, forced=force_balance or guarantee_min_days, force_any_duration=used_final_fallback)
                progress = True
            if not progress:
    ...
    def fill_saturday_gap(max_iter=500):
        ...
        # Smart Balance: riduce aggressività degli swap da infrasettimanale a weekend
        # Se smart_balance è attivo, lo swap avviene solo se il sabato ha gap critico (copertura = 0)
        if smart_balance and weekend_guard:
            # Verifica se c'è un gap CRITICO (slot con domanda > 0 e copertura = 0)
            critical_gap = False
            for slot in slot_list:
                demand = demand_by_slot[target_day].get(slot, 0.0)
                coverage = current_coverage[target_day].get(slot, 0)
                if demand > 0 and coverage == 0:
                    critical_gap = True
                    break
            if not critical_gap:
                print(f"\n[LOOP] FILL SATURDAY GAP - Smart Balance: sabato non ha gap critici, skip")
                return
    ...
    def fill_sunday_gap(max_iter=500):
        ...
        # Smart Balance: riduce aggressività degli swap da infrasettimanale a weekend
        # Se smart_balance è attivo, lo swap avviene solo se la domenica ha gap critico (copertura = 0)
        if smart_balance and weekend_guard:
            # Verifica se c'è un gap CRITICO (slot con domanda > 0 e copertura = 0)
            critical_gap = False
            for slot in slot_list:
                ...
                if demand > 0 and coverage == 0:
                    ...
                    break
            if not critical_gap:
                print(f"\n[LOOP] FILL SUNDAY GAP - Smart Balance: domenica non ha gap critici, skip")
                return
...
def process_single_skill_group(req, turni, ris, cfg, args, skill_label):
    ...
    # Politica Giorni Minimi
    weekday_overcap_max = None
    if args.weekday_overcap_max is not None:
        if args.weekday_overcap_max > 0:
            weekday_overcap_max = args.weekday_overcap_max / 100.0
    ...
    assignments, riposi_info, infeasible, day_colmap, slot_list, slot_size, shift_slots, assignment_details, ot_minutes_used = assegnazione_tight_capacity(
        ...
        guarantee_min_days=args.guarantee_min_days,
        smart_balance=args.smart_balance,
        overstaff_distributed=args.overstaff_distributed,
        weekday_overcap_max=weekday_overcap_max,
    )
...
def main(argv=None):
    ...
    parser.add_argument('--guarantee-min-days', action='store_true', help='Garantisce giorni minimi contrattuali')
    parser.add_argument('--smart-balance', action='store_true', help='Overstaff preferisce giorni infrasettimanali (protegge weekend)')
    parser.add_argument('--overstaff-distributed', action='store_true', help='Overstaff distribuito proporzionalmente su tutti i giorni')
    parser.add_argument('--weekday-overcap-max', type=float, default=None, help='Max overcoverage infrasettimanale in percent')
